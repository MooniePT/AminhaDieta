...
\begin{itemize}
    ...
    \item Definir metas de peso e acompanhar a evolução com alertas de pesagem.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Redesign da Interface}: Adoção de uma paleta de cores vibrante (Azul/Ciano) e tipografia moderna (Verdana).
    \item \textbf{Menu de Definições}: Implementação de um ecrã de configurações para permitir ao utilizador escolher entre um fundo dinâmico ("Arco-íris") ou estático, e alterar o tipo de letra.
    \item \textbf{Gamificação}: Introdução de mensagens motivacionais (Alertas) ao atingir metas de hidratação e registar exercícios.
    \item \textbf{Correções Visuais}: Ajuste de layouts e remoção de artefactos visuais (barras pretas) nos componentes JavaFX.
\end{itemize}

\subsection{Fase 6: Funcionalidades Avançadas (Semana 10)}
\begin{itemize}
    \item \textbf{Histórico de Peso}: Implementação de registo periódico (Semanal/Mensal) e visualização gráfica da evolução.
    \item \textbf{Metas de Peso}: Funcionalidade para definir um peso alvo e visualizar uma linha de meta no gráfico de histórico.
    \item \textbf{Notificações Inteligentes}: Sistema de alertas para lembrar o utilizador de se pesar e mensagens de celebração ao atingir o peso ideal.
    \item \textbf{Instalador Windows}: Criação de um pacote de instalação (`.exe`) profissional utilizando o `Inno Setup`, permitindo uma distribuição fácil e "amigável" da aplicação final.
...
\begin{itemize}
    \item \textbf{Atributos}: Nome, idade, peso, altura, género, nível de atividade, meta de peso e frequência de pesagem.
...
\begin{itemize}
    ...
    \item \textbf{Gráficos}: Apresenta um gráfico de linhas com a evolução do peso e uma linha horizontal indicando a meta definida.
    \item \textbf{Filtros}: Permite filtrar os dados por intervalo de datas.
    \item \textbf{Exportação PDF}: Integra com a biblioteca OpenPDF para gerar um documento físico, incluindo agora as metas de peso e frequência.
\end{itemize}
